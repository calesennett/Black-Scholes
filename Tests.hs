...
main = hspec $ do
    ...
    describe "blackScholes" $ do
        it "uses call function for a call option" $ do
            let option = BS.Option {BS.optionType="call", BS.stock=31.25, BS.strike=22.75, BS.riskFree=0.01, BS.time=1, BS.vol=0.5}
        ...
        it "uses put function for a put option" $ do
            let option = BS.Option {BS.optionType="put", BS.stock=31.25, BS.strike=35.00, BS.riskFree=0.01, BS.time=1, BS.vol=0.5}
                d1 = ((log (BS.stock o / BS.strike o)) + ((BS.riskFree o + (BS.vol o)^2 / 2.0) * (BS.time o))) / ((BS.vol o) * sqrt (BS.time o))
                d2 = d1 - (BS.vol o) * sqrt (BS.time o)
                o  = option
        ...
        it "can compute value on expiration date" $ do
            let option = BS.Option {BS.optionType="call", BS.stock=31.25, BS.strike=22.75, BS.riskFree=0.01, BS.time=0.0, BS.vol=0.5}
            BS.blackScholes option `shouldSatisfy` within 0.001 8.5

        it "can compute value with risk-free rate of 0" $ do
            let option = BS.Option {BS.optionType="call", BS.stock=31.25, BS.strike=22.75, BS.riskFree=0.0, BS.time=1, BS.vol=0.5}
            BS.blackScholes option `shouldSatisfy` within 0.001 10.5892

        it "can compute value with volatility of 0" $ do
            let option = BS.Option {BS.optionType="call", BS.stock=31.25, BS.strike=22.75, BS.riskFree=0.01, BS.time=1, BS.vol=0.0}
            BS.blackScholes option `shouldSatisfy` within 0.001 8.7264

        it "can compute a worthless option on expiration date" $ do
            let option = BS.Option {BS.optionType="call", BS.stock=31.25, BS.strike=35.00, BS.riskFree=0.01, BS.time=0.0, BS.vol=0.5}
